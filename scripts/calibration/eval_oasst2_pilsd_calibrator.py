"""F-T1 OASST2 PILSD calibrator replication - within-author EB-shrinkage RMSE gain.
...
5. Held-out predictions on test fold across 4 arms:
   ...
   - PILSD_EB_shrunk: alpha_shrunk_j + beta_shrunk_j * x  (the headline arm)
6. Cluster-bootstrap by user_id (Cameron 2008, Efron 1987 BCa); B=4000 reps default.
7. Pre-registered verdict assignment per `PREREG_F_T1_OASST2_REPLICATION.md` 2.6.
...
# Pre-registered priors (per PREREG_F_T1_OASST2_REPLICATION.md 1)
PREREG_PRIORS = {
...
# Pre-registered verdict bands (per PREREG 2.6)
PASS_LO = 5.0
PASS_HI = 12.0
PARTIAL_LO = 1.0
PARTIAL_HI = 5.0
...
# ============================================================================
# 1 PILSD core (verbatim from eval_user_score_mse_shrunk.py + F-T1 5corpus)
# ============================================================================
...
def cluster_bootstrap_gain_ci(sq_err_baseline, sq_err_pilsd, per_user_idx,
                              B=4000, seed=42):
    """Cluster-resample-by-user_id bootstrap on gain_pct = (rmse_base - rmse_pilsd)
...
# ============================================================================
# 3 Per-seed driver: PILSD on OASST2
# ============================================================================


def run_pilsd_oasst2(parquet_path, seed=42, k_folds=5, min_obs_per_user=6,
                    bootstrap_B=4000, bootstrap_seed=42):
    """Run the full PILSD pipeline on OASST2 scored parquet for a single seed.
    ...
    per_user_rows = []
    sq_err_per_row = {"no_calib": [], "pop_slope": [], "pilsd_ols": [], "pilsd_shrunk": []}
    ...
    # Wilcoxon signed-rank (per-user RMSE pop-slope vs PILSD-shrunk)
    try:
        if "rmse_pop_slope" in pu.columns and "rmse_pilsd_shrunk" in pu.columns:
            mask = pu[["rmse_pop_slope", "rmse_pilsd_shrunk"]].notna().all(axis=1)
            if mask.sum() > 0:
                w = scstats.wilcoxon(pu.loc[mask, "rmse_pop_slope"],
                                     pu.loc[mask, "rmse_pilsd_shrunk"],
                                     alternative="greater")
    ...
    bs = cluster_bootstrap_gain_ci(
        sq_err_baseline_per_row, sq_err_pilsd_per_row, per_row_uid,
...
def classify_verdict(gain_pct, ci_lo, ci_hi):
    """Apply pre-registered decision rule per PREREG 2.6."""
...
def main():
    ...
    print(f"[F-T1 OASST2] PREREG anchor: PREREG_F_T1_OASST2_REPLICATION.md")
    ...
    for seed in args.seeds:
        ...
        result["preReg_anchor"] = "PREREG_F_T1_OASST2_REPLICATION.md"
    ...
    summary = {
        ...
        "joint_verdict": joint,
        "preReg_priors": PREREG_PRIORS,
        "preReg_anchor": "PREREG_F_T1_OASST2_REPLICATION.md",
