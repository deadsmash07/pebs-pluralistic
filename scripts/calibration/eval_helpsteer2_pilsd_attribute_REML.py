"""REML verification of HelpSteer2 attribute-as-rater PILSD (iter+N+286).

Purpose
-------
The iter+N+266 HelpSteer2 claim (+15.92% pooled RMSE reduction, CI
...
Harville 1977 JASA 72:320). The iter+N+285 adversarial-pulse raised
ADV_285_A: if canonical REML (restricted maximum likelihood) pulls
...
eval_helpsteer2_pilsd_attribute.py, and reports:
...
Refit the PILSD predictions using REML-estimated tau^2 in place of MoM,
...
def fit_per_attribute_ols(train: pd.DataFrame) -> dict:
    """Replicate eval_helpsteer2_pilsd_attribute.py per-attr OLS."""
    out = {}
...
def evaluate_with_tau(train, test, tau_alpha_sq, tau_beta_sq, label):
    """Recompute pilsd_shrunk predictions with supplied tau^2 values."""
    ...
    for a in ATTRS:
        ...
        per_attr_rmse[a] = {
            ...
            "pilsd_shrunk": float(np.sqrt(np.mean((y_eb - y) ** 2))),
        }
...
def main():
    ...
    # Same 80/20 split (seed=42) as eval_helpsteer2_pilsd_attribute.py
    ...
    # Load MoM baseline from iter+N+266 eval.json
    ...
    print(f"\n[reml] MoM re-run gain  = {mom_eval['rel_impr_pct']:+.3f}%  "
          f"(matches iter266 {gain_pct_mom:+.3f}%?)")
    ...
    summary = {
        "iter": "N+286",
        "purpose": "REML verification of iter+N+266 HelpSteer2 MoM headline",
        "adversarial_attack": "ADV_285_A (section-level-paper-numerical-risk)",
