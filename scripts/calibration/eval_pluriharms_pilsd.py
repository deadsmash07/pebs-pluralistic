"""PluriHarms cross-dataset PILSD eval with bootstrap RMSE CIs.
...
"PRISM-only" reviewer attack by showing the PILSD EB-shrunk calibrator
...
- 4 arms (same as PRISM eval_user_score_mse_shrunk):
    ...
    3. pilsd_ols       : per-user (alpha_j, beta_j) via OLS (no shrinkage)
    4. pilsd_shrunk    : per-user EB shrunk toward (alpha_pop, beta_pop)
...
- Paired Wilcoxon (pilsd_shrunk vs each baseline, alt=less).
...
Outputs
-------
results/track1_pluriharms_pilsd/eval.json
...
OUT_DIR = ROOT / "results" / "track1_pluriharms_pilsd"
...
def predict_arms(
    ...
    if np.isnan(a):
        ...
        y_pilsd = a_eb * x + b_eb
    return {"no_calib": y_no, "pop_slope": y_pop, "pilsd_ols": y_ols, "pilsd_shrunk": y_pilsd}
...
def user_cluster_bootstrap(
    ...
    boot_rel_impr: list = []  # pilsd_shrunk vs pop_slope
    for _ in range(n_boot):
        ...
        # % reduction pilsd vs pop
        p = means["pop_slope"]
        v = means["pilsd_shrunk"]
        if p > 0:
    ...
    return {
        **{f"rmse_{a}_boot": _ci(boot[a]) for a in arms},
        "rel_impr_pilsd_vs_pop_pct_boot": _ci(boot_rel_impr),
    }
...
def main():
    ...
    shrunk_mean = point["pilsd_shrunk"]["mean"]
    rel_impr = 100.0 * (pop_mean - shrunk_mean) / pop_mean

    # Wilcoxon paired (user-level) — shrunk vs each baseline, alt=less
    wilc = {}
    for base in ["no_calib", "pop_slope", "pilsd_ols"]:
        w = stats.wilcoxon(
            rmse["pilsd_shrunk"].values, rmse[base].values, alternative="less"
        )
        wilc[f"pilsd_shrunk_vs_{base}"] = {
            "stat": float(w.statistic),
            "p": float(w.pvalue),
            "frac_shrunk_lt_base": float(
                (rmse["pilsd_shrunk"] < rmse[base]).mean()
            ),
        }
    ...
    summary = {
        ...
        "rmse": point,
        "rel_impr_pilsd_vs_pop_pct_point": float(rel_impr),
        "wilcoxon": wilc,
    ...
    print(
        f"\nrel_impr pilsd_shrunk vs pop_slope (mean): {rel_impr:+.3f}% "
        f"boot 95% CI [{boot['rel_impr_pilsd_vs_pop_pct_boot']['ci95_lo']:+.3f}, "
        f"{boot['rel_impr_pilsd_vs_pop_pct_boot']['ci95_hi']:+.3f}]"
    )
