"""Time-varying (alpha_j(t), beta_j(t)) PILSD via Kalman filter on PRISM.
...
POS-T1.1:
  ...
  1. pilsd_static       — EB-shrunk (alpha_j, beta_j) fit ONCE on train-half
  2. pilsd_random_walk  — Kalman F=I (pure random walk), (q_alpha, q_beta, R)
                          estimated by POOLED MLE across all users on train
  3. pilsd_kalman       — Kalman F=diag(rho_alpha, rho_beta) AR(1),
...
Output:
  results/track1_pilsd_kalman/eval.json
  results/track1_pilsd_kalman/per_user.parquet
  results/track1_pilsd_kalman/beta_trajectories.parquet  (long-format)

Run:
  cd IMPLEMENTATION/1_Causal_RLHF
  python3 scripts/eval_pilsd_kalman.py
...
def parse_args():
    ...
    p.add_argument("--output-dir",
                   default="results/track1_pilsd_kalman")
...
def main():
    ...
    for uid, (x_tr, y_tr, x_te, y_te, a_ols, b_ols, Va, Vb) in user_splits.items():
        ...
        # (D) pilsd_random_walk Kalman with MLE-fit (q_a, q_b, R_rw)
        ...
        row["rmse_pilsd_random_walk"] = float(np.sqrt(np.mean((yh - y_te) ** 2)))
        ...
        # (E) pilsd_kalman AR(1) with MLE-fit (rho_a, rho_b, q_a, q_b, R)
        mu_T, _, _, traj_ar = kalman_filter(
            x_tr, y_tr, mu0, Sigma0, F_ar, Q_ar, R,
            mu_stationary=mu0, return_trajectory=True,
        )
        yh = mu_T[0] + mu_T[1] * x_te
        row["rmse_pilsd_kalman"] = float(np.sqrt(np.mean((yh - y_te) ** 2)))
    ...
    # ---- headline RMSE means ----
    arms = ["no_calib", "pop_slope", "pilsd_static",
            "pilsd_random_walk", "pilsd_kalman"]
    ...
    comparisons = {
        "kalman_vs_static":      paired("rmse_pilsd_kalman", "rmse_pilsd_static"),
        "kalman_vs_random_walk": paired("rmse_pilsd_kalman", "rmse_pilsd_random_walk"),
        "random_walk_vs_static": paired("rmse_pilsd_random_walk", "rmse_pilsd_static"),
    ...
    a_static = pu["rmse_pilsd_static"].to_numpy()
    a_kalman = pu["rmse_pilsd_kalman"].to_numpy()
    a_rw = pu["rmse_pilsd_random_walk"].to_numpy()
    ...
    if delta_mean_ks < 0 and comparisons["kalman_vs_static"]["wilcoxon_p"] < 0.05 and ci_ks[1] < 0:
        verdict = "HELPS: Kalman AR(1) beats static PILSD (significant)"
    elif delta_mean_ks > 0 and comparisons["kalman_vs_static"]["wilcoxon_p"] < 0.05 and ci_ks[0] > 0:
        verdict = "HURTS: Kalman AR(1) worse than static PILSD (significant)"
    else:
        verdict = "NULL: Kalman AR(1) not significantly different from static PILSD"
    print(f"\n=== VERDICT ===\n  {verdict}")
