"""Attribute-as-rater PEBS on HelpSteer2.

Each of the 5 HelpSteer2 attributes (helpfulness, correctness,
...
PEBS shrinks per-attribute (alpha_a, beta_a) toward the population mean
...
----
  ...
  - pebs_ols    : per-attribute (alpha_a, beta_a) without shrinkage
  - pebs_shrunk : per-attribute (alpha_a, beta_a) with EB shrinkage
...
B=2000 reps, for 95% CI on rel-impr (pebs_shrunk vs pop_slope).
...
def evaluate(train: pd.DataFrame, test: pd.DataFrame) -> dict:
    ...
    for a in ATTRS:
        ...
        sq_err["pebs_ols"][a] = (y_ols - y) ** 2
        sq_err["pebs_shrunk"][a] = (y_eb - y) ** 2
...
def cluster_bootstrap(eval_result: dict, n_boot: int, seed: int) -> dict:
    ...
    RMSE for each arm; return CI on rel-impr (pebs_shrunk vs pop_slope)."""
    ...
    return {
        "rmse_boot": {arm: {a: _ci(out[arm][a]) for a in ATTRS + ["overall"]}
                      for arm in arms},
        "rel_impr_pebs_vs_pop": {a: _ci(rel_per_attr[a])
                                  for a in ATTRS + ["overall"]},
    }


def per_attribute_wilcoxon(eval_result: dict) -> dict:
    """Row-level paired Wilcoxon per attribute (pebs_shrunk vs pop_slope)."""
    sq_err = eval_result["sq_err"]
    out = {}
    for a in ATTRS:
        sh = sq_err["pebs_shrunk"][a]
...
def main():
    ...
    rel_per_attr_point = {
        a: rel(res["rmse_per_attribute"]["pop_slope"][a],
               res["rmse_per_attribute"]["pebs_shrunk"][a])
        for a in ATTRS
    }
    rel_overall_point = rel(res["rmse_overall"]["pop_slope"],
                            res["rmse_overall"]["pebs_shrunk"])
    ...
    print(f"{'attr':14s}  {'no_calib':>9s}  {'pop_slope':>9s}  "
          f"{'pebs_ols':>9s}  {'pebs_shr':>9s}  {'rel%':>7s}  "
          f"{'boot_lo':>8s}  {'boot_hi':>8s}  {'wilc p':>10s}")
    for a in ATTRS:
        ci = boot["rel_impr_pebs_vs_pop"][a]
        w = wilc[a]
        print(f"{a:14s}  "
              f"{res['rmse_per_attribute']['no_calib'][a]:9.3f}  "
              f"{res['rmse_per_attribute']['pop_slope'][a]:9.3f}  "
              f"{res['rmse_per_attribute']['pebs_ols'][a]:9.3f}  "
              f"{res['rmse_per_attribute']['pebs_shrunk'][a]:9.3f}  "
    ...
    print(f"\n{'overall':14s}  "
          ...
          f"{res['rmse_overall']['pebs_ols']:9.3f}  "
          f"{res['rmse_overall']['pebs_shrunk']:9.3f}  "
          f"{rel_overall_point:+7.3f}  "
          f"{boot['rel_impr_pebs_vs_pop']['overall']['ci95_lo']:+8.3f}  "
          f"{boot['rel_impr_pebs_vs_pop']['overall']['ci95_hi']:+8.3f}")
