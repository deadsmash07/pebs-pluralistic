"""3-backbone PEBS eval on PluriHarms.

Parallel to `llama32_3b_backbone/eval_pebs_3backbones.py` which handles
...
Outputs
-------
- `results/pluriharms_pebs_3backbones.json`
- `results/pluriharms_pebs_3backbones.parquet` (per-user RMSE table)

Anchor sources
--------------
- qwen7b: `Harm_Level` column from `data/pluriharms/prompts.csv` (existing
  protocol; serves as the Qwen-equivalent anchor since the original run
...
def parse_args():
    ...
    p.add_argument("--output-path",
                   default="results/pluriharms_pebs_3backbones.json")
    return p.parse_args()
...
def bootstrap_gain_ci(per_user_df: pd.DataFrame, name: str,
    ...
    gain% = 100 * (mean_pop_slope - mean_pebs_shrunk) / mean_pop_slope
...
def main():
    ...
    for name in backbone_specs:
        ...
        arms.append(f"pebs_shrunk_{name}")
    ...
    for uid, grp in df.groupby("user_id"):
        ...
        for train_idx, test_idx in kfold_split(n, args.k_folds, rng):
            ...
            for name in backbone_specs:
                ...
                yh = a_s + b_s * x_te
                sq[f"pebs_shrunk_{name}"].extend(((yh - y_te) ** 2).tolist())
    ...
    for name in backbone_specs:
        ...
        shrunk = float(pu[f"rmse_pebs_shrunk_{name}"].mean())
        rel = 100.0 * (base - shrunk) / base
        ca = pu[f"rmse_pebs_shrunk_{name}"].to_numpy()
