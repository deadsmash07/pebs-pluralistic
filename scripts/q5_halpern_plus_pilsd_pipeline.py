"""Q5 — Halpern (2025) FSAM + PILSD pipeline composition on PRISM LOCO.

Per ``memory/p0_experiment_queue_v1_2026_05_03.md`` Q5 brief: compose Halpern's
selection-style ensemble (Forward-Stagewise Additive Modeling over K reward
heads with simplex mixture weights) with PILSD's calibration-side per-rater
empirical-Bayes shrinkage. The brief frames the contribution as a
...
PILSD calibrates per-rater on the output side. If combined > both individual,
this supports the "PILSD complements selection-style methods" framing per the
user's 2026-05-02 17:00 IST efficiency-as-contribution-axis pattern, and
addresses friend #2's critique that PILSD is solely a calibration trick.
...
Q1, Q3, PReF, LoRe baseline conventions) are:
  ...
  2. **PILSD-only** — canonical W-B5 PILSD: per-user OLS calibrator on raw
     ``rm_score``, joint α+β EB shrinkage with closed-form ω_α=τ²_α/(τ²_α+V_α),
     ω_β=τ²_β/(τ²_β+V_β). VERBATIM port from
     ``paper/scripts/wave_b/wave_b_W_B5_half_pilsd.py:124-271`` "canonical" arm.
  3. **COMBINED-pipeline** — Halpern's per-utterance ensemble prediction
     replaces ``rm_score`` as the upstream signal feeding into PILSD's per-user
     OLS calibrator. PILSD's α+β shrinkage is then applied on (Halpern_pred,
     score_user). This tests: when the upstream signal is already
     persona-aware (FSAM K=4 ensemble), does PILSD's per-rater calibration
...
==================================================================
ESTABLISHED-PIPELINE-COMPOSITION-DOMINATES
...
MODERATE-PIPELINE-EQUIVALENT
    ...
    that PILSD does NOT degrade Halpern's ensemble signal).
...
FALSIFIED-PIPELINE-WORSE
    ...
    would be a SPOTLIGHT-tier honest finding).

Per ``Skill: research-grade-code-audit-pre-launch`` v1 12 gates
================================================================
G1 math-vs-code:
    ...
    reward in [0, 1].
    PILSD: VERBATIM port from W-B5 ``wave_b_W_B5_half_pilsd.py:124-271``
    ...
    sigmoid(r_k(z_uttance)) replaces raw rm_score as the input to PILSD's
    per-user OLS. PILSD's shrinkage formula is unchanged; only the upstream
    feature axis differs.
G2 hypothesis-vs-design:
    Tests "do Halpern's selection-side and PILSD's calibration-side compose
    to a pipeline-level gain", NOT "does either dominate head-to-head".
    The 4-class verdict explicitly enumerates BOTH the positive
    (ESTABLISHED-PIPELINE-COMPOSITION-DOMINATES) AND the
    negative-honest-disclosure (FALSIFIED-PIPELINE-WORSE) branches per
    ``Skill: honest-disclosure`` 4-class STRICT.
G3 no silent-bypass:
    3 distinct compute paths -- Halpern arm uses ensemble; PILSD arm
    ...
    arm ≡ trivial baseline ⇒ combined ≡ PILSD).
...
G4 eval pipeline integrity:
    ...
    (Halpern's per-user calibration uses TRAIN-fold scalars only; PILSD's
    omega + alpha_j + beta_j are estimated from the same TRAIN-fold).
G5 reference-implementation parity:
    HALPERN: FSAM math is paper-text re-implementation (no official
    code available; verified WebSearch + WebFetch 2026-05-02 19:00 IST
    per ``1_Causal_RLHF/scripts/baselines/halpern_2025_on_prism.py:38-40``).
    Existing T1 Halpern (file above) verdict: PRELIMINARY-HALPERN-BASELINE-
    COMPLETE rmse=0.3226 95% CI [0.3171, 0.3265] on n=1396 users with K=4
    ...
    PILSD canonical: VERBATIM port from W-B5 (verdict ESTABLISHED-COMPOUND-
    NEEDED canonical=+8.55%); ω formula = Efron-Morris (1973) eq. 2.9.
G6 hyperparameter sanity:
...
G7 per-arm diagnostic:
    Per-arm rmse_mean, gain_pct vs pop_slope baseline, 95% CI on per-user
    gain_pct via cluster-bootstrap, paired Δ vs PILSD-only and vs
...
G9 output schema:
    4-class STRICT verdict-class for the pipeline-composition aggregate
    (ESTABLISHED-PIPELINE-COMPOSITION-DOMINATES / MODERATE-PIPELINE-
    EQUIVALENT / PRELIMINARY-INCONCLUSIVE / FALSIFIED-PIPELINE-WORSE)
    per ``Skill: honest-disclosure`` 6.3.
G10 compute envelope:
    PURE NumPy + scipy on PRISM 1394 users; FSAM K=4 with closed-form
    polynomial fit per stage is O(N * K * D^2); K=4 D=3 N~7000 ⇒ wall
    estimate ~2-5 min CPU. The brief budgets ~6-12h GPU but that was
    sized for a neural-RM Halpern reproduction; this script's polynomial
    surrogate is faster + auditable + apples-to-apples with PReF/LoRe.
    HONEST-DISCLOSURE: this script is CPU-bound. GPU saturation < 5%
    expected; selecting h100_v2_backup is for cron-NEVER-IDLE discipline
    per user 2026-05-03 ~01:10 IST: "always all gpus must be running p0
    experiments"; alternative was deferring until a 7B GPU job queues up.
G11 anti-overfitting:
    ...
    small TRAIN folds. PILSD's EB shrinkage IS the anti-overfitting
...
G12 honest-disclosure:
    FALSIFIED-PIPELINE-WORSE branch ENUMERATED explicitly; would be a
    SPOTLIGHT-tier honest finding (the methods would be COMPETITIVE rather
    ...
    re-validation of Halpern's standalone performance (which T1 already did).

Output: ``results/track1_q5_halpern_plus_pilsd_pipeline/{summary.json}``
...
- T1 Halpern script: ``1_Causal_RLHF/scripts/baselines/halpern_2025_on_prism.py``
  (verdict PRELIMINARY-HALPERN-BASELINE-COMPLETE rmse=0.3226 95% CI [0.3171, 0.3265]).
- W-B5 PILSD canonical: ``paper/scripts/wave_b/wave_b_W_B5_half_pilsd.py``
  (verdict ESTABLISHED-COMPOUND-NEEDED canonical=+8.55% [+7.79, +9.31]).
- Q3 Half-PILSD cross-corpus: ``paper/scripts/q3_half_pilsd_cross_corpus.py``
  (verdict MODERATE-COMPOUND-NEEDED-1-CORPUS PRISM sanity reproduces W-B5).
...
DELTA_DOMINATE_PP = 1.0         # ESTABLISHED requires combined > best by 1pp
DELTA_EQUIVALENT_PP = 1.0       # MODERATE requires combined within 1pp of best
...
# G3 numerical-stability guard (inherited from W-B5 / Q3)
...
# ============================================================================
# G8 reproducibility helpers (verbatim from Q3 / Q1 conventions)
# ============================================================================
...
#
# Q5 polynomial-surrogate (G5 honest-disclosure caveat):
...
#   re-validation (T1 already validated Halpern at PRELIMINARY-class).
...
# ============================================================================
# 3-arm pipeline: Halpern-only / PILSD-only / Combined-pipeline
# ============================================================================
...
def run_three_arm_pipeline(
    ...
    # ------------------------------------------------------------------
    # G3 / G5 design correction (caught by Q5 smoke test):
    # ------------------------------------------------------------------
    ...
    # user calibration). The COMBINED arm THEN applies PILSD's per-user
    ...
    for uid, grp in df.groupby("user_id"):
        ...
        sq = {"pop_slope": [], "halpern_only": [], "pilsd_only": [], "combined": []}
        ...
        for train_idx, test_idx in folds:
            ...
            # ---------------- HALPERN-only arm ----------------
            # G5 design: HALPERN-only uses the GLOBAL Halpern fit (Theta_global,
            ...
            # (which would be the PILSD contribution). G3 silent-bypass
            # GUARDED: combined ≠ halpern_only because combined applies
            # PILSD per-user OLS + EB shrinkage that halpern_only lacks.
            ...
            y_hat_pilsd = a_s_p + b_s_p * x_te_raw
            sq["pilsd_only"].extend(((y_hat_pilsd - s_te) ** 2).tolist())

            # ---------------- COMBINED-pipeline arm ----------------
            # G3 silent-bypass GUARDED: pipeline composition uses Halpern's
            # GLOBAL ensemble prediction (h_pred_unit_te in [0, 1]) as the
            # upstream feature, then applies PILSD's per-user OLS + EB
            # shrinkage on top. This is the honest pipeline composition:
            # Halpern provides the persona-aware ensemble signal at the
            # POPULATION level (via FSAM K=4 mixture); PILSD then calibrates
            ...
            # HALPERN-only is ENTIRELY the per-user PILSD calibration.
            ...
            # For G3 stability we estimate fold-level population on combined
    ...
    arms = ["halpern_only", "pilsd_only", "combined"]
    ...
    # Paired deltas: combined vs halpern_only AND combined vs pilsd_only
    deltas: dict[str, dict[str, Any]] = {}
    combined_gains = (
        100.0 * (pu.rmse_pop_slope - pu.rmse_combined) / pu.rmse_pop_slope
    ).to_numpy()
    for arm in ("halpern_only", "pilsd_only"):
        arm_gains = (
            100.0 * (pu.rmse_pop_slope - pu[f"rmse_{arm}"]) / pu.rmse_pop_slope
        ).to_numpy()
...
# ============================================================================
# Verdict assignment (4-class STRICT per Skill: honest-disclosure 6.3)
# ============================================================================
...
def assign_verdict(out: dict[str, Any]) -> dict[str, Any]:
    ...
    Decision-rule:
      ESTABLISHED-PIPELINE-COMPOSITION-DOMINATES:
        Combined paired Δ vs BOTH halpern_only and pilsd_only at 95% CI strictly
    ...
    return {
        ...
        "delta_combined_vs_pilsd_pp": d_vs_p["delta_pct"],
        "delta_combined_vs_halpern_ci95_lo": d_vs_h["delta_ci95_lo"],
        "delta_combined_vs_halpern_ci95_hi": d_vs_h["delta_ci95_hi"],
        "delta_combined_vs_pilsd_ci95_lo": d_vs_p["delta_ci95_lo"],
        "delta_combined_vs_pilsd_ci95_hi": d_vs_p["delta_ci95_hi"],
        "verdict_class": verdict,
        "decision_rule": (
            "ESTABLISHED if combined paired Δ vs BOTH individual at 95% CI "
            "strictly positive AND combined ≥ best-of-individual + 1pp; "
            "FALSIFIED if combined paired Δ vs BOTH at 95% CI strictly "
            "negative AND combined ≤ best-of-individual - 1pp; "
            "MODERATE if combined within 1pp of best-of-individual; "
            "PRELIMINARY otherwise (CIs too wide / inconsistent dominance)."
        ),
    }
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--output-dir",
        default=str(ROOT / "results" / "track1_q5_halpern_plus_pilsd_pipeline"),
    )
...
def main() -> int:
    ...
    log(f"Q5 Halpern + PILSD pipeline composition starting")
    ...
    summary = {
        "experiment": "Q5_halpern_plus_pilsd_pipeline_composition",
        "protocol": (
            "k=5 random-fold within-user CV; 3 arms (halpern_only, pilsd_only, "
    ...
    log("")
    log(f"=== Q5 Halpern + PILSD pipeline composition complete (wall={wall_seconds:.1f}s) ===")
