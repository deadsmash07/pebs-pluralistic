"""Q9 — Mixed-corpus PEBS pooled across PRISM + PluriHarms + HelpSteer2 + OASST2.

Produces the pooled-multi-corpus PEBS headline. The hypothesis is that PEBS
pooled across 4 RLHF preference corpora (with corpus-level fixed-effect
normalization) demonstrates a positive headline gain. This is a STRONGER
generalization claim than per-corpus replication (Q1 HelpSteer2 + Q3
PluriHarms + W-B5/PRISM canonical + Wave-C OASST2 already held
individually): pooled => "PEBS on 4 RLHF preference corpora" rather than
"PEBS on PRISM with N replications".
...
while preserving per-cluster within-corpus structure that PEBS exploits.
...
HelpSteer2 cluster-type asymmetry: as in Q1, HelpSteer2 has no stable per-
...
pooled gain is a generalization claim about PEBS's CALIBRATION MECHANISM (per-
cluster EB shrinkage of intercept+slope) rather than a uniform per-rater claim.

Outcome classification
======================
CONFIRMED-PEBS-POOLED-4-CORPUS-DOMINATES   pooled gain ≥3pp + CI excludes 0
                                              (cluster-bootstrap by user-within-corpus)
PARTIAL-PEBS-POOLED-PARTIAL                 pooled gain in [1, 3]pp + CI excludes 0
TENTATIVE-INCONCLUSIVE-POOLED               CI straddles 0
REJECTED-POOLED-DEGRADES                     pooled < 0 with CI excluding 0
                                              OR
                                              ...
                                              a 0.5pp absolute threshold
                                              tripped trivially under moderate
                                              per-corpus heterogeneity

Design notes
============
- Math: PEBS math is VERBATIM from eval_oasst2_pebs_calibrator.py
...
- Hypothesis: tests "does pooled-corpus PEBS beat pooled pop-slope
...
- No silent-bypass: 5 distinct compute paths (4 per-corpus + 1 pooled);
   the quick-run test (--smoke) asserts non-zero pooled gain and per-corpus
   alpha/beta variance.
- Eval pipeline integrity: same RMSE-on-held-out-CV-fold metric as W-B5 / Q1 /
   eval_oasst2_pebs_calibrator.py / wave_c_exp2_oasst2_replication.py.
   Cluster-bootstrap by cluster_id_namespaced.
- Reference implementation: math + bootstrap inherit verbatim from
   eval_oasst2_pebs_calibrator.py:90-280 (the canonical OASST2
...
- Diagnostics: per-corpus n_clusters_post_filter, tau_alpha_sq,
...
- Reproducibility: SEED + bootstrap seed pinned + git HEAD persisted +
   parquet sha256 per corpus persisted in summary.json.
- Output schema: pooled-headline outcome +
   per-corpus arm rmse / gain / CI in companion parquet.
- Compute envelope: pure NumPy + scipy on ~1394 PRISM + ~100 PluriHarms +
    ~11244 HelpSteer2 + ~2049 OASST2 = ~14787 clusters. Estimated wall ~10-20
    min CPU; GPU not needed.
...
Output
------
results/track1_pooled_4_corpus_pebs/{summary.json, per_corpus_arms.parquet}
...
- W-B5 PRISM headline (canonical=+8.55%; `scripts/wave_b/wave_b_W_B5_half_pebs.py`).
- Q1 HelpSteer2 (gain=+3.700% [+3.346, +4.136]; `scripts/q1_helpsteer2_replication.py`).
- Q3 PluriHarms (canonical +9.66% [+7.28, +11.19], REVERSED half-arm pattern;
  `scripts/q3_half_pebs_cross_corpus.py`).
- OASST2 replication (canonical PEBS gain on quality target;
  `scripts/calibration/eval_oasst2_pebs_calibrator.py`).
"""
...
# Numerical-stability guard (verbatim from Q3; backported into W-B5)
V_ALPHA_BETA_FLOOR = 1e-6       # if Va or Vb < this, treat as degenerate (omega=0)
# (Q9-specific) ROBUST tau estimation: filter the per-cluster stats pool to
...
# 3800x correction; PEBS downstream gain went from -429% to a sensible value).
# Without this filter, the broken pipeline forces omega_b ~ 0 universally
# (tau_beta_sq capped at the 1e-6 floor) and PEBS predictions explode for
...
# Outcome thresholds
ESTABLISHED_GAIN_LO_PP = 3.0    # CONFIRMED if pooled gain >= 3pp + CI excl 0
MODERATE_GAIN_LO_PP = 1.0       # PARTIAL if 1-3pp + CI excl 0
MODERATE_GAIN_HI_PP = 3.0
N_CLUSTERS_FOR_HEADLINE = 500   # gate on n_clusters_post_filter (matches Q1)

# REJECTED-POOLED-DEGRADES branch ("REJECTED-POOLED-
# DEGRADES if pooled < per-corpus best (heterogeneity dominates)").
#
# After empirical validation (3-corpus real-data local run) we found that
# this per-corpus comparator is mis-specified: the per-corpus breakdown
# inside the pooled run is NOT the same as the standalone per-corpus PEBS
# result. The per-corpus breakdown evaluates POOLED PEBS predictions on a
# corpus-subset of held-out folds; this is mechanically NOT the same as a
# standalone per-corpus PEBS calibrator (Q3 PluriHarms +9.66%
# stand-alone is the correct comparator, NOT the +27% per-corpus slice from
# pooled-PEBS). The per-corpus slice gain reflects how much pooled-PEBS
# helps WITHIN a corpus, not how a standalone-per-corpus PEBS would do.
#
...
# Q9's REJECTED branch ONLY fires on UNAMBIGUOUS harm:
...
# The per-corpus-slice gain is
# a DIAGNOSTIC, not a per-corpus standalone benchmark; pooled results are
# cross-validated against the Q3 / Q1 / W-B5 / OASST2 standalone results
# after the run.
...
# ============================================================================
# Reproducibility helpers (verbatim from q3_half_pebs_cross_corpus.py)
# ============================================================================
...
# ============================================================================
# CORE PEBS MATH (verbatim from eval_oasst2_pebs_calibrator.py + Q1 + Q3)
# ============================================================================
...
def cluster_bootstrap_gain_ci(
    ...
    is verbatim from eval_oasst2_pebs_calibrator.py:150-280) with cluster_id =
...
def load_oasst2(parquet: Path) -> pd.DataFrame:
    ...
    VERBATIM cluster + target convention from eval_oasst2_pebs_calibrator.py:11-13.
    """
    df = pd.read_parquet(parquet)
...
def apply_corpus_fixed_effects(corpora: list[pd.DataFrame]) -> tuple[pd.DataFrame, dict]:
    ...
    cluster structure that PEBS exploits.
...
# ============================================================================
# POOLED PEBS ESTIMATOR (canonical 4-arm pipeline on z-score scale)
# ============================================================================
...
def run_pooled_pebs(
    ...
    """Run canonical 4-arm PEBS pipeline on the pooled z-scored cluster pool.
    ...
    # (Q9-specific) ROBUST tau estimation: filter pool to RELIABLE clusters
    ...
    # (pre-filter) reported in summary.json so the filter's effect is verifiable.
    ...
    for cid, grp in df.groupby("cluster_id"):
        ...
        for train_idx, test_idx in folds:
            ...
            y_hat_ols = a + b * x_te
            # Numerical-stability guard (verbatim from the Q3 fix)
    ...
    pooled_gain_pct = ((rmse_baseline - rmse_pebs) / rmse_baseline * 100.0
    ...
    log(f"[pooled] rmse_pebs       = {rmse_pebs:.5f}")
    ...
    # Best per-corpus gain (for REJECTED-POOLED-DEGRADES branch)
    ...
    # AND relative (>50%) thresholds. Recorded but does NOT trigger REJECTED
    # by itself because per-corpus-slice gain is NOT the standalone per-corpus
    # PEBS reference; cross-validation against Q3/Q1/W-B5/F-T1 standalone
    # results happens after the run.
    ...
    if not np.isfinite(pooled_gain_pct) or not np.isfinite(pooled_ci_lo):
        ...
        verdict = "REJECTED-POOLED-DEGRADES"
    elif pooled_gain_pct >= ESTABLISHED_GAIN_LO_PP and ci_excludes_zero:
        verdict = "CONFIRMED-PEBS-POOLED-4-CORPUS-DOMINATES"
    elif (
        MODERATE_GAIN_LO_PP <= pooled_gain_pct < ESTABLISHED_GAIN_LO_PP
        and ci_excludes_zero
    ):
        verdict = "PARTIAL-PEBS-POOLED-PARTIAL"
    else:
        verdict = "TENTATIVE-INCONCLUSIVE-POOLED"
    ...
    # Sanity flag: tau-pool excluded >50% (indicates corpus-heterogeneity-
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--output-dir",
                   default=str(ROOT / "results" / "track1_pooled_4_corpus_pebs"))
    return p.parse_args()
...
def main() -> int:
    ...
    summary: dict[str, Any] = {
        "experiment_id": "Q9_pooled_4_corpus_pebs",
        "verdict_class": "PENDING",
        "args": vars(args),
        "anchors": {
            "w_b5_prism_result": "canonical=8.55%",
            "q1_helpsteer2_result": "gain=+3.700% [+3.346, +4.136]",
            "q3_pluriharms_result": "canonical=+9.66% [+7.28, +11.19] REVERSED-half-arm",
            "f_t1_oasst2_result": "canonical PEBS gain on quality target",
        },
        "honest_disclosure_design_notes": (
            ...
            "(<corpus>::<cluster>) before applying canonical PEBS. HelpSteer2 "
            "uses cluster=prompt_hash (no stable per-rater id per Wang et al. "
            "2024 §3.4); other 3 corpora use cluster=user_id. The pooled "
            "headline is a generalization claim about PEBS's CALIBRATION "
            "MECHANISM (per-cluster EB shrinkage) on a heterogeneous cluster "
            "pool, NOT a uniform per-rater claim. The REJECTED-POOLED-DEGRADES "
    ...
    for cname, loader_fn, anchor in loaders:
        ...
        try:
            c_df = loader_fn()
            if args.smoke:
                # Small quick-run per corpus to keep within minutes
                if cname == "helpsteer2":
    ...
    # ------------------------------------------------------------------
    # Phase 3 — Pooled PEBS on z-score scale
    # ------------------------------------------------------------------
    log(f"[Q9] === running pooled PEBS ===")
    result = run_pooled_pebs(
        pooled_df,
        seed=args.seed,
    ...
    # ------------------------------------------------------------------
    # Per-corpus arm parquet (for downstream analysis)
    # ------------------------------------------------------------------
    arm_rows = []
    for cname, c in result.get("per_corpus_breakdown", {}).items():
        arm_rows.append({
            ...
            "rmse_pebs": c.get("rmse_pebs", float("nan")),
    ...
    arm_rows.append({
        ...
        "rmse_pebs": result.get("rmse_pebs", float("nan")),
    ...
    print()
    print(f"=== Q9 Pooled 4-Corpus PEBS (PRISM + PluriHarms + HelpSteer2 + OASST2) ===")
