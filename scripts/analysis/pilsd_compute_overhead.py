"""PILSD computational-overhead benchmark.

Reviewer-priority-1 for NeurIPS T1+T3 paper: "is PILSD practical at deployment
...
Measures wall-clock + memory for:
  ...
  (c) PILSD-shrunk calibration (fit per-user (α_j, β_j), MoM τ², shrink, apply)
...
OUT = (Path(__file__).resolve().parents[2] / "3_PILSD_Standalone/results/track1_compute_overhead")
RNG = 20260420
...
def main():
    ...
    # backbone-dependent and orthogonal to PILSD).
    ...
    # PILSD-shrunk: per-user fit, MoM τ², shrinkage
    t0 = time.perf_counter()
    per_user = {}
    ...
    for uid, (a, b, sa, sb) in per_user.items():
        ...
        shrunk[uid] = (a_s, b_s)
    pilsd_fit_s = time.perf_counter() - t0

    # PILSD inference: lookup user calibrator + mul + add
    t0 = time.perf_counter()
    ...
    t0 = time.perf_counter()
    _ = a0 + b0 * xs_test
    pilsd_infer_s = time.perf_counter() - t0

    results["PILSD_shrunk"] = {
        "fit_wall_s": pilsd_fit_s,
        "fit_per_user_us": pilsd_fit_s * 1e6 / len(per_user),
        "infer_per_pred_ns": pilsd_infer_s * 1e9 / len(xs_test),
    ...
    print(f"\n[PILSD-shrunk] fit={pilsd_fit_s*1000:.2f}ms total, {pilsd_fit_s*1e6 / len(per_user):.1f}μs/user, "
          f"infer={pilsd_infer_s*1e9 / len(xs_test):.2f}ns/pred")

    # Overhead ratios
    overhead_fit = pilsd_fit_s / pop_fit_s
    overhead_infer = pilsd_infer_s / pop_infer_s
    ...
    proj_10k_users_fit_s = pilsd_fit_s * (10000 / len(per_user))
    ...
    summary = {
        ...
        "PILSD_shrunk": results["PILSD_shrunk"],
