"""PILSD pair-accuracy on PRISM held-out slice — raw vs affine vs quadratic.

Reviewer gap: concurrent work (LoRe 71%, PReF, etc.) reports pair-accuracy on
PRISM, while PILSD papers lead with RMSE reduction. This script closes the gap
...
results/track1_quadratic_calibrator/per_pair.parquet:
  ...
  2. PILSD-affine per-user (α_j, β_j) calibration
  3. PILSD-quadratic per-user (α_j, β_j, γ_j) calibration
...
OUT_DIR = (Path(__file__).resolve().parents[2] / "3_PILSD_Standalone/results/track1_pair_accuracy_comparison")
...
def main():
    ...
    estimators = [
        ("raw_7B",         "margin_raw"),
        ("pilsd_affine",   "margin_affine"),
        ("pilsd_quadratic","margin_quadratic"),
    ]
