"""PILSD gain stratified by PRISM demographics — fairness/robustness check.
...
This is BOTH a robustness check (PILSD should help users roughly
...
OUT_DIR = (Path(__file__).resolve().parents[2] / "3_PILSD_Standalone/results/track1_gain_by_demographic")
...
def main():
    ...
    for ax, (col, label) in zip(axes, columns_to_scan):
        ...
        ax.set_xlabel("PILSD gain (RMSE units)")
    fig.tight_layout()
    ...
    print(f"[fairness] subgroups with CI entirely BELOW zero (PILSD actively harms): "
