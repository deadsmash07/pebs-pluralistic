"""PILSD RMSE gain stratified by PRISM conversation type.

Sibling to iter+N+209's pair-acc-by-conv-type experiment, but tests the
calibration-sensitive RMSE metric (where PILSD's mechanism has leverage
...
OUT = (Path(__file__).resolve().parents[2] / "3_PILSD_Standalone/results/track1_rmse_by_convtype")
N_BOOT = 2000
RNG = 20260420
...
def main():
    ...
    df["score_pilsd"] = df["alpha_j"] + df["beta_j"] * df["rm_score"]
    df["sq_err_naive"] = (df["score_user"] - df["score_naive"]) ** 2
    df["sq_err_pilsd"] = (df["score_user"] - df["score_pilsd"]) ** 2
    ...
    def per_user_rmse_gain(sub: pd.DataFrame) -> pd.DataFrame:
        g = sub.groupby("user_id", observed=True).agg(
            ...
            rmse_pilsd=("sq_err_pilsd", lambda v: float(np.sqrt(v.mean()))),
            n_obs=("sq_err_naive", "size"),
        )
        g["gain_abs"] = g["rmse_naive"] - g["rmse_pilsd"]
    ...
    for ctype, sub in df.groupby("conv_type", observed=True):
        ...
        results.append({
            ...
            "mean_rmse_pilsd": float(np.sqrt(sub["sq_err_pilsd"].mean())),
    ...
    print("\n=== PILSD RMSE gain by conversation type ===")
    print(f"{'type':24s} {'n_utt':>7s} {'n_user':>7s} {'RMSE_naive':>11s} {'RMSE_pilsd':>11s} "
          f"{'gain%':>7s} {'CI95_gain%':>20s}")
    for r in results:
        print(f"{r['conv_type']:24s} {r['n_utterances']:>7d} {r['n_users']:>7d}  "
              f"{r['mean_rmse_naive']:>10.3f}  {r['mean_rmse_pilsd']:>10.3f}  "
