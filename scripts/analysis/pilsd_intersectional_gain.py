"""Intersectional PILSD gain: thin-user × non-English-native × non-male.

Combines iter+N+203 (demographic fairness) with iter+N+202 (gain-vs-n_j
by quintile). Addresses reviewer attack "your fairness audit is marginal;
intersectional subgroups might still be harmed."

Strategy: form the hardest subgroup for PILSD = small n_j AND minority
...
OUT_DIR = (Path(__file__).resolve().parents[2] / "3_PILSD_Standalone/results/track1_intersectional_gain")
...
def main():
    ...
    # Build 4 intersectional cells in order of "difficulty" for PILSD
    cells = []
    ...
    print("\n=== Intersectional gain cells (hardest PILSD subgroups) ===")
    results = []
