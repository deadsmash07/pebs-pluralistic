"""PILSD gain as a function of cohort size N.
...
MoM τ² on that sub-cohort, compute PILSD-shrunk gain vs pop-OLS.
...
OUT = (Path(__file__).resolve().parents[2] / "3_PILSD_Standalone/results/track1_gain_vs_cohort_size")
N_SEEDS = 30
RNG = 20260420
...
def run_subcohort(df_scored, users, rng):
    """Compute PILSD-shrunk gain on the cohort defined by `users`."""
    ...
    # "how well does PILSD calibrate within this cohort" given each user's
    # data is available)
    per_user = {}
    ...
    # For each user compute PILSD-shrunk predictions vs pop-OLS predictions on
