"""Cross-dataset generalization test: does PEBS's 8.58% RMSE improvement
...
def main():
    ...
    ap.add_argument("--output", default="results/t1_pebs_on_multipref.json")
    args = ap.parse_args()
    ...
    print(f"\n=== PEBS cross-dataset result (MultiPref) ===")
    ...
    print(f"RMSE PEBS (shrunk)  = {res['rmse_shrunk_pebs']:.5f}")
    ...
    prism_target = 8.58
    if ci_lo > 0:
        verdict = "POSITIVE (PEBS generalizes)"
    elif pct > 0:
