"""W-B5: Half-PILSD α-only / β-only / both ablation (gain decomposition).

This is a Pass-5 (gain decomposition) ablation that tests the paper's
narrative that the 8.58% RMSE-reduction headline = ~4.70pp Efron-Morris
intercept-shrinkage (α-only) + ~1.17pp PILSD-specific slope-shrinkage
...
within-user CV protocol:
  ...
  3. both (canonical PILSD): joint α+β shrinkage (the paper's method)

Verdict-class STRICT (per Skill ``honest-disclosure`` 4-class):
  * ESTABLISHED-COMPOUND-NEEDED — both α-only AND β-only strictly worse
    than canonical (gain delta ≤ -0.5pp at 95% CI) AND each ablation
    contributes ≥10% of canonical gain
  * MODERATE-COMPOUND-NEEDED — directional dominance but CIs overlap
    OR one component contributes <10% of canonical gain
  * PRELIMINARY-INCONCLUSIVE — CIs broad / outcome ambiguous
  * FALSIFIED-ONE-COMPONENT-DOES-IT-ALL — α-only OR β-only ≥ canonical
    within 0.5pp 95% CI (would be SPOTLIGHT-tier honest finding;
    simpler method beats compound; strengthens paper)

Per Skill ``research-grade-code-audit-pre-launch`` 12 gates:
  * G1 math: each arm computes a strict subset of the canonical
    PILSD shrinkage; α-only sets ω_β=0; β-only sets ω_α=0; canonical
    uses both. Verified line-aligned to eval_user_score_mse_shrunk.py
    lines 89-146.
  * G2 hypothesis-vs-design: this script tests "which component does
    the work", NOT "does PILSD beat baseline"; canonical gain is the
    reference point. Decision-rule: gain decomposition + 95% CI on
    paired delta vs canonical.
  * G3 no silent-bypass: each arm is a DISTINCT compute path —
    smoke-test asserts arm rmse values are different.
  * G4 eval pipeline integrity: same RMSE eval as canonical
  * G5 reference-implementation: canonical is eval_user_score_mse_shrunk.py;
    α-only / β-only are documented strict subsets per Efron-Morris 1973
  * G6 hyperparameter sanity: k_folds=5, min_obs_per_user=6, seed=20260420
    (matches canonical headline seed)
  * G7 per-arm diagnostic: per-arm rmse, per-arm gain_pct, paired Δ vs
    canonical, 95% bootstrap CI
  * G8 reproducibility: single seed=20260420 (matches headline)
  * G9 output schema: summary.json with arms[α_only, β_only, canonical] +
    deltas vs canonical + verdict_class (4-class STRICT)
  * G10 compute realism: 3 arms × ~3-5min = 9-15min wall on 64 vCPU CPU-only
  * G11 anti-overfitting: not theory-claiming
  * G12 honest-disclosure: FALSIFIED-ONE-COMPONENT-DOES-IT-ALL is a
    spotlight-tier honest finding, NOT a paper-killer; verdict-class
    explicitly enumerates this.

Output: ``results/track1_half_pilsd_ablation/{summary.json}``
"""
...
# G3 numerical-stability guard (backported 2026-05-03 from
# `paper/scripts/q3_half_pilsd_cross_corpus.py:156` per Skill:
# research-grade-code-audit-pre-launch v1 G3 + Q3 dispatch memo
# `memory/q3_half_pilsd_cross_corpus_dispatch_0553.md` §3 G3 row).
#
...
# canonical W-B5 PRISM result (T3 RESULTS `15a659d`) is unchanged. On
...
# per Q3's INCONCLUSIVE-DEGENERATE-X verdict-class scope-limit).
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        default=str(
            ...
            / "../results/track1_half_pilsd_ablation"
        ),
    )
...
def run_ablation(
    ...
    for uid, grp in df.groupby("user_id"):
        ...
        for train_idx, test_idx in folds:
            ...
            # rationale. Backported 2026-05-03 from Q3 cross-corpus script.
...
def assign_verdict(out: dict[str, Any]) -> dict[str, Any]:
    ...
    Decision-rule:
      * FALSIFIED-ONE-COMPONENT-DOES-IT-ALL — α-only OR β-only delta CI95
        upper bound ≥ -0.5pp (i.e., not strictly worse than canonical at 95%)
      * ESTABLISHED-COMPOUND-NEEDED — both α-only AND β-only strictly worse
        (delta CI95 upper < -0.5pp) AND each component contributes ≥10% of
        canonical gain
      * MODERATE-COMPOUND-NEEDED — both arms directionally worse but CIs
        overlap zero OR one component <10% of canonical
      * PRELIMINARY-INCONCLUSIVE — fallback for ambiguous cases
    """
    ...
    # FALSIFIED: at least one ablation matches canonical within 0.5pp at 95% CI
    one_does_it_all = (
        a_delta_hi >= -DELTA_ONE_DOES_IT_PP or b_delta_hi >= -DELTA_ONE_DOES_IT_PP
    )

    if one_does_it_all:
        verdict = "FALSIFIED-ONE-COMPONENT-DOES-IT-ALL"
    elif (
        ...
        verdict = "ESTABLISHED-COMPOUND-NEEDED"
    elif a_delta_mean < 0 and b_delta_mean < 0:
        verdict = "MODERATE-COMPOUND-NEEDED"
    else:
    ...
    return {
        ...
        "decision_rule": (
            "FALSIFIED if α-only OR β-only delta CI95 upper ≥ -0.5pp; "
            "ESTABLISHED if both arms strictly worse (delta CI95 upper < -0.5pp) "
            "AND each component contributes ≥10% of canonical gain; "
            "MODERATE if directionally consistent but CIs overlap zero OR one "
            "component <10%; PRELIMINARY otherwise."
        ),
    }
...
def main() -> int:
    ...
    summary = {
        "experiment": "W-B5_half_pilsd_ablation",
    ...
    print("\n=== W-B5 Half-PILSD ablation complete ===")
