"""Q3 — Half-PEBS α/β ablation EXTENDED to PluriHarms + HelpSteer2.

Extends the Wave-B half-PEBS gain-decomposition (W-B5,
`scripts/wave_b/wave_b_W_B5_half_pebs.py`) from a PRISM-only result to a
cross-corpus result. W-B5 on PRISM found:
...
A PRISM-only ablation leaves open whether the α/β decomposition replicates
on a different annotation schema. The cross-corpus extension addresses this:
if the same dominance pattern (α-only ≪ canonical AND β-only ≪ canonical,
both Δ CI excl 0) holds on PluriHarms + HelpSteer2, the decomposition
generalizes beyond PRISM (CONFIRMED-COMPOUND-NEEDED-CROSS-CORPUS).

Decision rule
=============
CONFIRMED-COMPOUND-NEEDED-CROSS-CORPUS    ≥ 2 of (PluriHarms, HelpSteer2)
                                             confirm PRISM pattern: canonical
                                             > α-only AND canonical > β-only,
                                             both deltas at 95% CI strictly
                                             exclude 0 (delta_ci95_hi < 0)
PARTIAL-COMPOUND-NEEDED-1-CORPUS           1 of 2 confirms (other corpus
                                             may have one delta CI straddling 0
                                             or partial confirmation)
TENTATIVE-INCONCLUSIVE-CROSS-CORPUS       0 of 2 confirm clearly (CIs
                                             too wide / inconsistent dominance)
REJECTED-DECOMPOSITION-PRISM-SPECIFIC      cross-corpus pattern REVERSES
...
Design notes
============
- Math: each arm computes a strict subset of canonical PEBS shrinkage.
  α-only sets ω_β=0 (uses pop_β); β-only sets ω_α=0 (uses pop_α); canonical
  uses both. Identical to the estimator in
  `scripts/wave_b/wave_b_W_B5_half_pebs.py:124-271`, with the
  user_id ↔ cluster_id substitution for HelpSteer2 per-prompt clustering.
- Hypothesis: tests "does the α/β decomposition story REPLICATE on
  non-PRISM corpora", NOT "does PEBS beat baseline". Two corpora; per-corpus
  3-arm outcome + cross-corpus aggregate. The REJECTED branch (mechanism is
  PRISM-specific) is enumerated explicitly.
- Sanity: 3 distinct compute paths per corpus; a quick-run check asserts
  per-arm RMSE values DIFFER (would catch a degenerate omega_β=0 forcing
  β-only ≡ canonical).
- Metric: same RMSE metric as W-B5 / canonical PRISM headline;
...
- Reference implementation: math + bootstrap inherit verbatim from
  `wave_b_W_B5_half_pebs.py` (ω_α = τ²_α / (τ²_α + V_α); ω_β = τ²_β /
  (τ²_β + V_β)) which itself ports from `eval_oasst2_pebs_calibrator.py:90-280`.
  Efron-Morris (1973) eq. 2.9; Henderson (1975) BLUP shrinkage.
- Hyperparameters: K_FOLDS=5, MIN_OBS_PER_CLUSTER=6, SEED=20260420
  (matches W-B5 + PRISM headline + Q1 conventions). N_BOOT=2000.
- Diagnostics: per-arm rmse_mean, per-arm gain_pct vs pop,
...
- Reproducibility: SEED=20260420 + N_BOOT=2000 + git HEAD persisted +
  parquet sha256 persisted in summary.json.
- Compute: pure NumPy + scipy on 1394 PRISM-equivalent + 100
  PluriHarms users + 11244 HelpSteer2 prompts. Estimated wall ~10-15 min CPU;
  GPU not needed.
- Caveats: the REJECTED branch is enumerated explicitly (a PRISM-specific
  decomposition would scope-limit the paper claim); per-corpus n_obs
  distribution caveats logged (PluriHarms ~150 obs/user; HelpSteer2
  ~6 obs/cluster — Morris small-cluster bound predicts gain attenuation on
  HelpSteer2; expect smaller Δ magnitudes than PRISM).

Output: `results/track1_half_pebs_cross_corpus/{summary.json, per_corpus_arms.parquet}`

References
----------
- W-B5 PRISM: `scripts/wave_b/wave_b_W_B5_half_pebs.py`
- Q1 HelpSteer2: `scripts/q1_helpsteer2_replication.py`
  (gain +3.700% [+3.346, +4.136])
- W-B5 PRISM result: results/track1_half_pebs_ablation/summary.json
  (canonical 8.55%, α-only 7.46%, β-only 0.74%)
...
DELTA_ONE_DOES_IT_PP = 0.5      # matches W-B5 (REJECTED threshold)
# Numerical-stability guard for HelpSteer2's near-degenerate within-cluster
...
# This guard is INERT on PRISM (where the W-B5 result is unchanged) and on
...
# ============================================================================
# Reproducibility helpers (verbatim from q1_helpsteer2_replication.py)
# ============================================================================
...
def ols_with_V(x: np.ndarray, y: np.ndarray) -> tuple[float, float, float, float]:
    ...
    VERBATIM from wave_b_W_B5_half_pebs.py:110-121 identical to the estimator there."""
    k = len(x)
...
def run_ablation_one_corpus(
    ...
    """Run 3-arm half-PEBS ablation on one corpus.
    ...
    # Per-corpus outcome (matches W-B5 assign_verdict)
    ...
    # cannot be tested). Scope-limit rather than falsify.
    no_shrinkage_signal = (
    ...
    if no_shrinkage_signal:
        ...
        # scope-limit rather than retract (NOT a falsification
        ...
        per_corpus_verdict = "REJECTED-ONE-COMPONENT-DOES-IT-ALL"
    elif (
        ...
        per_corpus_verdict = "CONFIRMED-COMPOUND-NEEDED"
    elif a_delta_mean < 0 and b_delta_mean < 0:
        per_corpus_verdict = "PARTIAL-COMPOUND-NEEDED"
    else:
        per_corpus_verdict = "TENTATIVE-INCONCLUSIVE"
...
# ============================================================================
# CROSS-CORPUS OUTCOME AGGREGATION
# ============================================================================

def assign_cross_corpus_verdict(per_corpus: dict[str, dict]) -> dict[str, Any]:
    """Aggregate per-corpus outcomes into a cross-corpus outcome.

    Decision rule:
      CONFIRMED-COMPOUND-NEEDED-CROSS-CORPUS    ≥2 of 2 corpora confirm
                                                   CONFIRMED-COMPOUND-NEEDED
      PARTIAL-COMPOUND-NEEDED-1-CORPUS           1 of 2 confirms
                                                   CONFIRMED-COMPOUND-NEEDED;
                                                   the other is PARTIAL-* or
                                                   TENTATIVE-*
      TENTATIVE-INCONCLUSIVE-CROSS-CORPUS       0 confirm CONFIRMED;
                                                   pattern is directionally
                                                   present but CIs straddle
      REJECTED-DECOMPOSITION-PRISM-SPECIFIC      ≥1 corpus shows
                                                   REJECTED-ONE-COMPONENT-
    ...
    for cname, c in per_corpus.items():
        ...
        if v == "CONFIRMED-COMPOUND-NEEDED":
            n_established += 1
        elif v == "PARTIAL-COMPOUND-NEEDED":
            n_moderate += 1
        elif v == "REJECTED-ONE-COMPONENT-DOES-IT-ALL":
            n_falsified += 1
    ...
    # REJECTED dominates if ANY corpus reverses the pattern (genuine reversal,
    # not a degenerate-x null)
    if n_falsified >= 1:
        cross_verdict = "REJECTED-DECOMPOSITION-PRISM-SPECIFIC"
    elif n_testable == 0:
        ...
        cross_verdict = "CONFIRMED-COMPOUND-NEEDED-CROSS-CORPUS"
    elif n_established >= N_CORPORA_FOR_MODERATE:
        cross_verdict = "PARTIAL-COMPOUND-NEEDED-1-CORPUS"
    elif (n_established + n_moderate) >= N_CORPORA_FOR_MODERATE:
        cross_verdict = "PARTIAL-COMPOUND-NEEDED-1-CORPUS"
    else:
        cross_verdict = "TENTATIVE-INCONCLUSIVE-CROSS-CORPUS"
    ...
    return {
        ...
        "decision_rule": (
            f"CONFIRMED-COMPOUND-NEEDED-CROSS-CORPUS if ≥{N_CORPORA_FOR_ESTABLISHED} "
            f"corpora confirm CONFIRMED; PARTIAL if ≥{N_CORPORA_FOR_MODERATE}; "
            f"REJECTED if any corpus reverses pattern (one component does all the work); "
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--include-prism", action="store_true",
                   help="Include PRISM as a 3rd corpus (sanity-check; should "
                        "match the W-B5 result).")
    p.add_argument("--smoke", action="store_true",
                   help="50-cluster smoke per corpus.")
    p.add_argument("--output-dir",
                   default=str(ROOT / "results" / "track1_half_pebs_cross_corpus"))
    return p.parse_args()
...
def main() -> int:
    ...
    summary: dict[str, Any] = {
        "experiment_id": "Q3_half_pebs_cross_corpus",
        "verdict_class": "PENDING",
        "args": vars(args),
        "anchors": {
            "w_b5_prism_result": "canonical=8.55%",
            "q1_helpsteer2_result": "gain=+3.700%",
        },
        "honest_disclosure_design_notes": (
            ...
            "corpus outcome respects this asymmetry: the pattern is "
            "'canonical > {α-only, β-only}' not absolute magnitudes. REJECTED "
            ...
            "real work). PARTIAL-COMPOUND-NEEDED on a corpus simply means "
    ...
    # Design choice: PRISM excluded from cross-corpus aggregate even if --include-
    ...
    print()
    print(f"=== Q3 Half-PEBS Cross-Corpus Ablation (PluriHarms + HelpSteer2) ===")
