"""Unification figure across the three RLHF settings.

This script builds the single capstone figure that communicates the
ONE-LINE thesis:

    PEBS = partial-pooling / random-effect accounting dominates naive
    aggregation across three RLHF settings:
      (A) reward-model calibration              (PRISM)
      (B) asynchronous credit assignment        (3-state MDP)
      (C) longitudinal drift detection          (reviewer cohort)

Panels
------
A. Calibration learning curve: mean held-out RMSE vs per-user budget k.
...
B. Gradient-bias bars: ||E[grad] - grad_true|| across delay horizons
   Delta in {5, 20, 50}. Bars: naive r_fast-only (red), RAC (teal).
   Log-y scale. Mean reduction factor (47.9x) annotated.

C. Composition-shift diagnostics. Left sub-group: FPR on pure
...
All three panels use the consistent color code:
    ...
    teal (#17becf) = PEBS (partial-pooling, EB shrinkage, RAC, MixedLM)
...
- Simpson stress (pure composition shift, true drift = 0):
  3_PEBS_Standalone/results/track3_simpson_stress/simpson_stress.json
- Mixed drift (composition + genuine drift):
  3_PEBS_Standalone/results/track3_mixed_drift/mixed_drift.json

Outputs
-------
- 3_PEBS_Standalone/results/figs/unified_pebs_principle.png  (150 DPI)
- 3_PEBS_Standalone/results/figs/unified_pebs_principle.svg  (vector)
- 3_PEBS_Standalone/results/unified_pebs_summary.json       (machine-readable)

Usage
-----
    python3 scripts/plot_unified_pebs_principle.py
...
DEFAULT_TRACK3_SIMPSON = REPO_ROOT / "3_PEBS_Standalone/results/track3_simpson_stress/simpson_stress.json"
DEFAULT_TRACK3_MIXED = REPO_ROOT / "3_PEBS_Standalone/results/track3_mixed_drift/mixed_drift.json"

DEFAULT_OUTPUT_PNG = REPO_ROOT / "3_PEBS_Standalone/results/figs/unified_pebs_principle.png"
DEFAULT_OUTPUT_SVG = REPO_ROOT / "3_PEBS_Standalone/results/figs/unified_pebs_principle.svg"
DEFAULT_SUMMARY_JSON = REPO_ROOT / "3_PEBS_Standalone/results/unified_pebs_summary.json"

# -----------------------------------------------------------------------------
# Consistent color palette across panels
# -----------------------------------------------------------------------------
COLOR_NAIVE = "#d62728"   # strong red         -> naive aggregation / OLS / r_fast only
COLOR_PEBS = "#17becf"   # strong teal        -> partial pooling / shrinkage / RAC / MixedLM
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Build the cross-track unified PEBS principle figure.",
    )
...
def plot_panel_a(ax, naive: Dict[str, Any], shrunk: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Extract per-k stats
    naive_mean = np.array([per_k_naive[str(k)]["rmse_pebs"]["mean"] for k in budgets])
    naive_median = np.array([per_k_naive[str(k)]["rmse_pebs"]["median"] for k in budgets])
    naive_p25 = np.array([per_k_naive[str(k)]["rmse_pebs"]["p25"] for k in budgets])
    naive_p75 = np.array([per_k_naive[str(k)]["rmse_pebs"]["p75"] for k in budgets])
    ...
    ax.plot(
        ...
        color=COLOR_PEBS, linewidth=2.4, marker="s", markersize=7,
        label="EB-shrunk (median)  [PEBS]",
    )
    ...
    ax.axvline(5, color=COLOR_PEBS, linestyle="--", linewidth=1.0, alpha=0.6)
    ax.axvline(20, color=COLOR_NAIVE, linestyle="--", linewidth=1.0, alpha=0.6)
    ax.annotate(
        "shrunk break-even\nk=5",
        xy=(5, pop_slope_rmse), xytext=(2.4, pop_slope_rmse - 5.2),
        fontsize=9, color=COLOR_PEBS,
        arrowprops=dict(arrowstyle="->", color=COLOR_PEBS, lw=1.0),
        ha="center",
    )
    ax.annotate(
        "naive break-even\nk=20",
...
def plot_panel_b(ax, track2: Dict[str, Any]) -> Dict[str, Any]:
    ...
    b2 = ax.bar(x + bar_w / 2, bias_rac, width=bar_w,
                color=COLOR_PEBS, edgecolor="black", linewidth=0.6,
                label="RAC forward-injected  [PEBS]")
...
# -----------------------------------------------------------------------------
# Panel C : composition-shift diagnostics
# -----------------------------------------------------------------------------
def plot_panel_c(ax, simpson: Dict[str, Any], mixed: Dict[str, Any]) -> Dict[str, Any]:
    ...
    colors = [COLOR_PEBS, COLOR_NAIVE, COLOR_PH]
    ...
    ax.set_title("C. Longitudinal drift detection\n"
    ...
    legend_handles = [
        mpatches.Patch(color=COLOR_PEBS, label="MixedLM+perm  [PEBS]"),
...
def main() -> None:
    ...
    fig.suptitle(
        "PEBS: partial-pooling / random-effect accounting dominates naive aggregation "
    ...
    summary = {
        ...
        "paper_caption": (
            "Figure N. PEBS's partial-pooling principle dominates naive "
            "aggregation across three RLHF settings. "
    ...
    print("=" * 72)
    print("Unified PEBS principle figure written to:")
    print(f"  PNG : {args.output_png}")
    print(f"  SVG : {args.output_svg}")
    print(f"  JSON: {args.summary_json}")
    print("=" * 72)
    print("Panel A :  break-even k   shrunk={}   naive={}   "
    ...
    print("Panel B :  mean reduction = {}x    min = {}x    mean VIF = {}".format(
        summary["track2"]["mean_bias_reduction_factor"],
        summary["track2"]["min_bias_reduction_factor"],
        summary["track2"]["mean_vif"],
    ))
    print("Panel C :  FPR  ml={}  naive={}  ph={}    sign-correct  ml={}/{}  naive={}/{}".format(
